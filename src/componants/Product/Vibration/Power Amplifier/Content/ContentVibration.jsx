...
const HEAD = {
  ...
  section1: {
    ...
    desc3:
      "Assembled in industry standard modular racks, all modules of our Power Amplifier are rack mountable. Removing and installing modules is made easy via guide rails on which every module is designed to slide. Designed to meet international design and safety standards, every module of our Power Amplifier is feature rich and contains its own power entry, power filtering, error indication and EMI conforming systems. Continuous upgrade of technology to better meet the international standards and reduce the carbon footprint of our system is a never-ending process at DAS. Through our various training, services and maintenance programs, we keep our customers updated with the latest product offerings and upgrades.",
  },
  section2: {
    heading: "Digital Logic Unit",
    desc: "The microcontroller based Digital Logic Unit (DLU) controls and monitors the system features and safety interlocks of the DSA series Power Amplifier. The logic unit acts as a user friendly interface between the user and the equipment and can be operated both in manual and auto modes.",
    desc2:
  ...
  section7: {
    ...
    desc: "All shakers from DAS have dynamic auto centring except DS 125 and DS 140, which features a mechanical only auto centring. Models DS 180 to DS 760 feature a combination of pneumatic + DC bias based auto centring. The mechanical axial stiffness of these models is nearly negligible. A displacement sensor is used calculate the exact position of armature in real-time. Based on this data, a digital control loop utilises air diaphragm and bellows for pneumatic auto centring and an analog control loop utilises DC bias for the Power Modules for DC bias auto centring.",
  },
  section8: {
...
section9: {
    ...
    desc1: "Every power module is rack mounted and features a wire free design implemented using copper bus bars instead of wires. This ensures that our power module never faces any issues pertaining to loose or damaged connection. Each power module of the DSA series has a built-in PWM generation and management. These modules are stacked in master slave configuration for their parallel operation in the amplifier. PWM in paralleled modules is synchronised through master clock and a current feedback network is also implemented for precise current sharing amongst modules. Three-sigma design ensures peak current capability for continuous random operation, without affecting the life of the power device.",
